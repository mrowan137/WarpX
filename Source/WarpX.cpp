...
WarpX::WarpX ()
{
    ...
    costs_heuristic.resize(nlevs_max);
...
WarpX::ReadParameters ()
{
...

    {
        ...
        pp.query("load_balance_type", load_balance_type_str);
        load_balance_type = MapGetWithDefault(load_balance_type_str_to_load_balance_type,
                                              load_balance_type_str,
                                              LBType::UNDEFINED);
        pp.query("costs_heuristic_cell_wt", costs_heuristic_cell_wt);
        pp.query("costs_heuristic_prtl_wt", costs_heuristic_prtl_wt);
                      ...
                      const IntVect& ngF, const IntVect& ngextra, const bool aux_is_nodal)
{
    ...
    if (load_balance_int > 0) {
        ...
        costs_heuristic[lev].reset(new MultiFab(ba, dm, 1, 0));
    }
}
